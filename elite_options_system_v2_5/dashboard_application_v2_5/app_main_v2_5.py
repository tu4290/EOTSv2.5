...
try:
    ...
    try:
        ...
        if not global_config_manager_from_runner._config:
            if global_config_manager_from_runner.load_config("config_v2_5.json", "config.schema.json"):
...
# --- Fallback Layout Function (if layout_manager_v2_5 fails to import) ---
def _fallback_get_main_layout_v2_4(app_config: Optional[Dict[str,Any]] = None) -> html.Div:
    error_msg = "APP_MAIN FALLBACK: Main layout_manager_v2_5 module failed to import or its layout function is unavailable. Displaying minimal fallback content."
    ...
    return html.Div([
        ...
        html.P("Please check the application logs for detailed error messages from 'layout_manager_v2_5.py' or 'app_main.py'.", style={'textAlign': 'center'}),
        html.Hr(),
...
imported_layout_manager_v2_5_module: Optional[Any] = None
get_dashboard_layout_func_ref: Callable = _fallback_get_main_layout_v2_4
try:
    from . import layout_manager_v2_5 as imported_layout_manager_v2_5_module
    if hasattr(imported_layout_manager_v2_5_module, 'get_main_dashboard_layout_v2_4'):
        get_dashboard_layout_func_ref = imported_layout_manager_v2_5_module.get_main_dashboard_layout_v2_4
        logger_app_main.info("APP_MAIN: Successfully imported get_main_dashboard_layout_v2_4 from .layout_manager_v2_5.")
    else:
        logger_app_main.error("APP_MAIN ERROR: Function 'get_main_dashboard_layout_v2_4' not found in .layout_manager_v2_5. Using fallback layout.")
except ImportError as e_lm_appmain:
    logger_app_main.critical(f"APP_MAIN CRITICAL: Failed to import .layout_manager_v2_5 module: {e_lm_appmain}. Using fallback layout.", exc_info=True)

imported_callback_manager_v2_5_module: Optional[Any] = None
register_all_callbacks_func_ref: Optional[Callable] = None
try:
    from . import callback_manager_v2_5 as imported_callback_manager_v2_5_module
    register_all_callbacks_func_ref = imported_callback_manager_v2_5_module.register_all_callbacks
    logger_app_main.info("APP_MAIN: Successfully imported register_all_callbacks from .callback_manager_v2_5.")
except ImportError as e_cb_man_am:
    logger_app_main.critical(f"APP_MAIN CRITICAL: Failed to import .callback_manager_v2_5: {e_cb_man_am}. Callbacks will not be registered.", exc_info=True)
...
def initialize_dashboard_dependencies(
    ...
    logger_app_main.info("Step 4: Populating registered chart IDs from layout_manager_v2_5...")
    if imported_layout_manager_v2_5_module and hasattr(imported_layout_manager_v2_5_module, 'populate_all_registered_chart_ids'):
        try:
            chart_ids_found = imported_layout_manager_v2_5_module.populate_all_registered_chart_ids(_CONFIG_MANAGER_SHARED)
            num_chart_ids = len(chart_ids_found) if chart_ids_found is not None else 0
            logger_app_main.info(f"Step 4 complete: layout_manager_v2_5.populate_all_registered_chart_ids called. Found {num_chart_ids} chart IDs.")
        except Exception as e_pop_ids:
            logger_app_main.error(f"APP_MAIN: Error calling layout_manager_v2_5.populate_all_registered_chart_ids: {e_pop_ids}", exc_info=True)
